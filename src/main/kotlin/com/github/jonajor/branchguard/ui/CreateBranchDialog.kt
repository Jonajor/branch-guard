...
class CreateBranchDialog(
    ...
    private val prefixOptions = listOf("feature", "bugfix", "hotfix", "fix", "chore", "refactor")

    private val prefixCombo = ComboBox(
        prefixOptions.toTypedArray()
    ).apply {
        val default = settings.branchPrefix.trim().ifBlank { "feature" }
        val idx = prefixOptions.indexOf(default)
    ...
    override fun createCenterPanel(): JComponent {
        ...
        ticketField.document.addDocumentListener(docListener)
        descriptionField.document.addDocumentListener(docListener)
        prefixCombo.addItemListener { e ->
            if (e.stateChange == ItemEvent.SELECTED) updatePreview()
        }

        val messagePanel = JPanel(BorderLayout()).apply {
            border = JBUI.Borders.emptyBottom(8)
            add(
                JBLabel("<html>You are editing on: <b>$currentBranch</b><br>Create a branch before continuing?</html>"),
                BorderLayout.CENTER,
            )
        }
    ...
    private fun currentResult() = BranchNameGenerator.generate(
        BranchNameInput(
            ...
            prefix = prefixCombo.selectedItem as? String ?: "feature",
